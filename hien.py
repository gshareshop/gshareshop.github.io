...
def getItems(url_path="0", tq="select A,B,C,D,E"):
	...
	for row in _json["table"]["rows"]:
		...
		# Náº¿u phÃ¡t hiá»‡n spreadsheet khÃ¡c vá»›i GshareMedia
		...
		if "plugin://" in item["path"]:
			if "install-repo" in item["path"]:
				item["is_playable"] = False
			elif re.search("plugin.video.salemmax.hien/(.+?)/.+?\://", item["path"]):
				match = re.search(
					"plugin.video.salemmax.hien(/.+?/).+?\://", item["path"])
	...
	if url_path == "0":
		add_playlist_item = {
			...
			"label":"[B][COLOR cyan]===>[COLOR blueviolet]ThÃªm Playlist[/COLOR]<===[/B][/COLOR]",
...
def AddTracking(items):
	...
	for item in items:
		if "plugin.video.salemmax.hien" in item["path"]:
